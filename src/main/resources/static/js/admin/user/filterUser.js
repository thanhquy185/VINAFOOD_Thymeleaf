...
// Các biến giữ đối tượng cho việc lọc "Người dùng"
const filterButton = document.querySelector(
  ".main__filter button#filter-button-user"
);
...
const roleSelect = document.querySelector(".main__filter input#role-select");
...
export function filterEvents() {
  if (filterButton && findInput && sortSelect && roleSelect && statusSelect) {
    ...
    if (sessionStorage.length >= 1) {
      ...
      roleSelect.value = sessionStorage.getItem("role") || "";
      ...
      if (roleSelect.value) {
        selectEvent03(
          ".main__filter input#role-select + ul li",
          roleSelect.value
        );
      }
    ...
    filterButton.addEventListener("click", (e) => {
      ...
      const roleValue = String(roleSelect.value).trim();
      ...
      if (findValue) {
        ...
        searchParams.set("fullname", findValue);
        searchParams.set("phone", findValue);
      }
      if (sortValue) {
        searchParams.set("sort", sortValue);
      }
      if (roleValue) {
        searchParams.set("role", roleValue);
      }
      if (statusValue) {
